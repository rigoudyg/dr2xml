...
def process_vertical_interpolation(sv, alias, src_grid_id):
    ...
    if sv.type in ["dev", ]:
        ...
        if len(vdims) > 1:
            logger.error("Too many vertical dims for %s (%s)" % (sv.label, repr(vdims)))
            ...
            # else:
            logger.error("Not enough vertical dims for %s (%s)" % (sv.label, [(s.label, s.out_name)
                                                                              for s in sv.sdims.values()]))
            raise Dr2xmlError("Not enough vertical dims for %s (%s)" %
                              (sv.label, [(s.label, s.out_name) for s in sv.sdims.values()]))
        else:
            ...
            if alias_with_levels in pingvars:
                ...
                add_value_in_dict_config_variable(variable="field_defs", key=alias_for_sampling,
                                                  value=DR2XMLElement(tag="field", id=alias_for_sampling,
                                                                      field_ref=alias_in_ping, operation=operation))
                #
                vert_freq = internal_dict["vertical_interpolation_sample_freq"]
                #
                # Construct an axis for interpolating to this vertical dimension
                # e.g. for zoom_case :
                #    <axis id="zoom_plev7h_hus" axis_ref="union_plevs_hus">
                #    <zoom_axis index="(0,6)[  3 6 11 13 15 20 28 ]"/>
                #    </axis>
                create_axis_def(sd)
                ...
                # <field id="CMIP6_hus_sampled_3h" field_ref="CMIP6_hus_instant" freq_op="3h"
                # expr="@CMIP6_hus_instant"/>
                add_value_in_dict_config_variable(variable="field_defs", key=alias_sample,
                                                  value=DR2XMLElement(tag="field",
                                                                      text="@{}".format(alias_for_sampling),
                                                                      id=alias_sample, field_ref=alias_for_sampling,
                                                                      detect_missing_value="true", freq_op=vert_freq))
                ...
                if sd.is_zoom_of:  # cas d'une variable definie grace a 2 axis_def (union+zoom)
                    ...
                    # e.g. <grid id="FULL_klev_zoom_plev7h_hus">
                    # <domain domain_ref="FULL" />
                    # <axis axis_ref="zoom_plev7h_hus" />
                    # </ grid>

                    # cas d'une variable definie grace a 2 axes verticaux (zoom+union)
                    # Must first create grid for levels union, e.g.:
                    # <grid id="FULL_klev_union_plevs_hus">
                    # <domain domain_ref="FULL" />
                    # <axis axis_ref="union_plevs_hus" />
                    # </ grid>
                    grid_id = create_grid_def(axis_defs[sd.is_zoom_of], sd.out_name, src_grid_id)
                    #
                    union_alias = alias_in_ping + "_union"  # e.g. 'CMIP6_hus_union'
                    # Ss e.g.: <field id="CMIP6_hus_union" field_ref="CMIP6_hus_sampled_3h"
                    #            grid_ref="FULL_klev_union_plevs_hus"/>
                    add_value_in_dict_config_variable(variable="field_defs", key=union_alias,
                                                      value=DR2XMLElement(tag="field", id=union_alias,
                                                                          field_ref=alias_sample, grid_ref=grid_id))

                    # SS : first create grid for levels subset zoom, e.g.:
                    # <grid id="FULL_klev_zoom_plev7h_hus">
                    # <domain domain_ref="FULL" />
                    # <axis axis_ref="zoom_plev7h_hus" />
                    # </ grid>
                    # e.g. zoom_label : 'zoom_plev7h_hus'
                    grid_id = create_grid_def(axis_defs[sd.zoom_label], sd.out_name, src_grid_id)
                    # SS: e.g.: <field id="CMIP6_hus7h_plev7h" field_ref="CMIP6_hus_union"
                    #             grid_ref="FULL_klev_zoom_plev7h_hus" />
                    add_value_in_dict_config_variable(variable="field_defs", key=alias_with_levels,
                                                      value=DR2XMLElement(tag="field", id=alias_with_levels,
                                                                          field_ref=union_alias, grid_ref=grid_id))

                else:  # cas d'une variable definie grace a seul axis_def (non union+zoom)
                    # Construct a grid using variable's grid and target vertical axis
                    union_alias = False
                    axis_key = sd.label  # e.g. 'plev7h'
                    grid_id = create_grid_def(axis_defs[axis_key], sd.out_name, src_grid_id)
                    add_value_in_dict_config_variable(variable="field_defs", key=alias_with_levels,
                                                      value=DR2XMLElement(tag="field", id=alias_with_levels,
                                                                          field_ref=alias_sample, grid_ref=grid_id))
                    # if "hus" in alias :
...
def process_zonal_mean(field_id, grid_id, target_hgrid_id, zgrid_id, operation, frequency):
    """
    ...
    is for optimization when both contexts occur
    ...
    field1_def = DR2XMLElement(tag="field", id=field1_id, field_ref=field_id, operation=operation)
    add_value_in_dict_config_variable(variable="field_defs", key=field1_id, value=field1_def)
    logger.debug("+++ field1 %s\n%s" % (field1_id, field1_def))
    #
    ...
    field2_def = DR2XMLElement(tag="field", text="@{}".format(field1_id), id=field2_id, field_ref=field1_id,
                               freq_op=xios_freq)
    add_value_in_dict_config_variable(variable="field_defs", key=field2_id, value=field2_def)
    logger.debug("+++ field2 %s\n%s" % (field2_id, field2_def))
    ...
    if target_hgrid_id:  # case where an intermediate grid is needed
        ...
        grid_id3 = change_domain_in_grid(target_hgrid_id, src_grid_id=grid_id)
        logger.debug("+++ grid3 %s\n%s" % (grid_id3, get_config_variable("grid_defs")[grid_id3]))
        field3_def = DR2XMLElement(tag="field", id=field3_id, field_ref=field2_id, grid_ref=grid_id3)
        add_value_in_dict_config_variable(variable="field_defs", key=field3_id, value=field3_def)
        logger.debug("+++ field3 %s\n%s" % (field3_id, field3_def))
    else:
    ...
    grid4_id = change_domain_in_grid(zgrid_id, src_grid_id=grid_id3, turn_into_axis=True)
    logger.debug("+++ grid4 %s\n%s" % (grid4_id, get_config_variable("grid_defs")[grid4_id]))
    field4_def = DR2XMLElement(tag="field", id=field4_id, field_ref=field3_id, grid_ref=grid4_id)
    add_value_in_dict_config_variable(variable="field_defs", key=field4_id, value=field4_def)
    logger.debug("+++ field4 %s\n%s" % (field4_id, field4_def))

    return field4_id, grid4_id


def process_diurnal_cycle(alias):
    """
    Based on a field id ALIAS, and global CONTEXT_INDEX, creates and
    ...
    field_for_average_def = DR2XMLElement(tag="field", id=field_for_average_id, field_ref=alias, operation="average")
    add_value_in_dict_config_variable(variable="field_defs", key=field_for_average_id, value=field_for_average_def)

    logger.debug("***> %s" % field_for_average_def)
    ...
    averaged_field_def = DR2XMLElement(tag="field", text="@{}".format(field_for_average_id), id=averaged_field_id,
                                       freq_op="1h", grid_ref=grid_scalar_id)
    add_value_in_dict_config_variable(variable="field_defs", key=averaged_field_id, value=averaged_field_def)
    logger.debug("***> %s" % averaged_field_def)
    ...
    add_value_in_dict_config_variable(variable="axis_defs", key=axis_24h_id, value=axis_24h)
    ...
    grid_24h.append(DR2XMLElement(tag="axis", axis_ref=axis_24h_id, name="time3", unit="days since ?",
                                  standard_name="time", axis_type="T"))
    add_value_in_dict_config_variable(variable="grid_defs", key=grid_24h_id, value=grid_24h)
    ...
    add_value_in_dict_config_variable(variable="field_defs", key=alias_24h_id, value=alias_24h)
    logger.debug("***> %s\n" % alias_24h)
...
def process_levels_over_orog(sv, alias, src_grid_id):
    ...
    if field_id in get_config_variable("pingvars"):
        ...
        if height_over_orog_field_name not in context_index:
            ...
            orography_grid_def = get_grid_def(orography_grid_id)
            if len(orography_grid_def) == 0:
                ...
                if len(orography_domain) != 1:
                    ...
                    if len(orography_grid_def) == 1:
                        ...
                        add_value_in_dict_config_variable(variable="grid_defs", key=orography_grid_scalar_id,
                                                          value=orography_scalar_grid_def)
                        orography_with_scalar_id = "_".join([orography_field_name, "scalar"])
                        add_value_in_dict_config_variable(variable="field_defs", key=orography_with_scalar_id,
                                                          value=DR2XMLElement(tag="field", text=orography_field_name,
            ...
            if hlev_grid_id not in context_index:
                ...
                hlev_axis = DR2XMLElement(tag="axis", id=height_axis_id,
                                          axis_ref=[elt for elt in get_grid_def(src_grid_id)
                                                    if elt.tag in ["axis", ]][0].attrib["axis_ref"])
                hlev_axis.append(DR2XMLElement(tag="duplicate_scalar"))
                hlev_grid_id = create_grid_def(hlev_axis, None, src_grid_id)
            ...
            add_value_in_dict_config_variable(variable="field_defs", key=orog_with_duplicate_id,
                                              value=orog_with_duplicate_def)
            ...
            add_value_in_dict_config_variable(variable="field_defs", key=height_over_orog_field_name,
                                              value=height_over_orog_field_def)
        ...
        if height_over_orog_axis_name not in context_index:
            ...
            add_value_in_dict_config_variable(variable="axis_defs", key=height_over_orog_axis_name,
                                              value=height_over_orog_axis_def)
        # Create the new axis
        if sd.requested:
        ...
        add_value_in_dict_config_variable(variable="axis_defs", key=axis_id, value=axis)
        # Create the new grid
        grid_id = create_grid_def(axis, sd.out_name, src_grid_id)
        # Build the field
        field = DR2XMLElement(tag="field", id=field_id, field_ref=alias_in_ping, grid_ref=grid_id)
        add_value_in_dict_config_variable(variable="field_defs", key=field_id, value=field)
