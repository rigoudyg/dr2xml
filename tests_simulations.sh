...
simulations_to_test=${5:-"a4SST amip land 1pctCO2 piNTCF piControl ssp585 hist_aer piClim aladin levels aladin_nodr RCSM6_HIS amip_ESM RCSM6_HIS_light C3S-SF"}
check_time=${6:-"no"}

if [ "${check_time}" = "yes" ]; then
  check_time_file="check_time.csv"
else
  check_time_file="no"
fi
if [ "${add_coverage}" = "yes" ]; then
  ...
  for simulation in $simulations_to_test; do
    coverage run --parallel-mode --source=$scripts_to_cover tests_simulations.py $simulation $run_mode $to_compare $add_profile $check_time_file
      if [ ! $? -eq 0 ] ; then
          exit 1
      fi
  done
  ...
  for simulation in $simulations_to_test; do
    python tests_simulations.py --simulation=$simulation --run_mode=$run_mode --to_compare=$to_compare --add_profile=$add_profile --check_time_file=$check_time_file
      if [ ! $? -eq 0 ] ; then
          exit 1
      fi
  done
